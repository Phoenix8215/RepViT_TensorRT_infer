...
Model::Model(string onnx_path, logger::Level level, model::Params params) {
...
bool Model::build_engine() {
    ...
    if (!parser->parseFromFile(m_onnxPath.c_str(), 1)) { return false; }
    ...
    shared_ptr<Int8EntropyCalibrator> calibrator(new Int8EntropyCalibrator(
        64, "calibration/calibration_list_updated.txt", "calibration/calibration_table.txt", 3 * 224 * 224, 224, 224));
    config->setInt8Calibrator(calibrator.get()); // get():获取裸指针

    auto engine =
        shared_ptr<ICudaEngine>(builder->buildEngineWithConfig(*network, *config), destroy_trt_ptr<ICudaEngine>);
...
void Model::print_network(INetworkDefinition& network, bool optimized) {
    ...
    if (!optimized) {
        for (int i = 0; i < layerCount; i++) {
            ...
            LOGV("layer_info: %-40s:%-25s->%-25s[%s]", layer->getName(), printTensorShape(input).c_str(),
                 printTensorShape(output).c_str(), getPrecision(layer->getPrecision()).c_str());
        }
