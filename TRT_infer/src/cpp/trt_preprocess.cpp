...
// 根据比例进行缩放 (CPU版本)
cv::Mat preprocess_resize_cpu(cv::Mat& src, const int& tar_h, const int& tar_w, float* d_mean, float* d_std,
                              tactics tac) {
    cv::Mat tar;
    ...
    /*Resize*/
    switch (tac) {
        case tactics::CPU_NEAREST: cv::resize(src, tar, cv::Size(resizeW, resizeH), 0, 0, cv::INTER_LINEAR); break;
        case tactics::CPU_BILINEAR: cv::resize(src, tar, cv::Size(resizeW, resizeH), 0, 0, cv::INTER_NEAREST); break;
        default: LOGE("ERROR: Wrong CPU resize tactics selected. Program terminated"); exit(1);
    }
    return tar;
}

// 根据比例进行缩放 (GPU版本)
void preprocess_resize_gpu(cv::Mat& h_src, float* d_tar, const int& tar_h, const int& tar_w, float* h_mean,
                           float* h_std, tactics tac) {
