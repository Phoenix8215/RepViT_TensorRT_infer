...
void Classifier::setup(void const* data, size_t size) {
    ...
    // 考虑到大多数classification model都是1 input, 1 output, 这边这么写。对于多输入头和多输出头的模型需要修改
    ...
    // 在host上分配pinned memory,device 上分配device memory
...
bool Classifier::preprocess_cpu() {
    ...
    cv::resize(input_image, input_image, cv::Size(m_params->img.w, m_params->img.h), 0, 0, cv::INTER_LINEAR);
    ...
    CUDA_CHECK(cudaMemcpyAsync(m_inputMemory[1], m_inputMemory[0], m_inputSize, cudaMemcpyKind::cudaMemcpyHostToDevice,
                               m_stream));
...
bool Classifier::preprocess_gpu() {
    ...
    process::preprocess_resize_gpu(input_image, m_inputMemory[1], m_params->img.h, m_params->img.w, mean, std,
                                   process::tactics::GPU_BILINEAR);
...
shared_ptr<Classifier> make_classifier(std::string onnx_path, logger::Level level, model::Params params) {
    auto classifier = make_shared<Classifier>(onnx_path, level, params);
    classifier->init_model(); // 资源开始的时候初始化一次就行了, 之后只需要推理
    return classifier;
}
